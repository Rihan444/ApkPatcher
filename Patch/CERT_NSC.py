...
-----END CERTIFICATE-----"""
    ...
    if CA_Cert:
        for idx, cert_path in enumerate(CA_Cert, start=1):
            raw_file = f'Rihan444{idx}.pem' if idx > 1 else 'Rihan444.pem'
        ...
        raw_path = M.os.path.join(raw_dir, 'Rihan444.pem')

        open(raw_path, 'w', encoding='utf-8', errors='ignore').write(Default_CERT)

        print(f"\n{C.S} Certificate {C.E}{C.G} The default certificate is from BHHC Infinity Tech Modded HttpCanary... ✔\n")

        print(f"\n{C.S} Write Default Certificate {C.E} {C.OG}➸❥ {C.Y}{M.os.path.basename(raw_dir)}/Rihan444.pem {C.G} ✔\n")
...
<network-security-config xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    if CA_Cert:
        for idx, _ in enumerate(CA_Cert, start=1):
            CERT_Name = f'Rihan444{idx}' if idx > 1 else 'Rihan444'
            cert_Entries += f'            <certificates src="@raw/{CERT_Name}" overridePins="true" />\n'

    else:
        cert_Entries += '            <certificates src="@raw/Rihan444" overridePins="true" />\n'
...
def update_public_xml(decompile_dir, CA_Cert):
    ...
    for line in public_xml:
        for match in raw_xml_regex.findall(line):
            ...
            if res_type == "raw" and res_name == "Rihan444":
                raw_found = True
    ...
    if not raw_found:
        if CA_Cert:
            ...
            new_entries.append(f'  <public id="0x{new_raw_id:08X}" type="raw" name="Rihan444" />\n')
